...
class AnnotationManager:
    ...
    def process_image(self) -> None:
        ...
        self.processed_img = self.img_processor.process_img(self.image_path, self.clicks, self.segmentation_map)
    ...
    def click_event(self, event, x, y, flags, params):
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            row, col = x, y  # Convert (x, y) to (row, col)
            ...
            if self.click_count % 2 == 0:
                cv2.line(self.img, (self.clicks[-1][0], self.clicks[-1][1]),
                         (self.clicks[-1][2], self.clicks[-1][3]), (255, 255, 255), 2)
            ...
            for idx, (lit_row, lit_col, shad_row, shad_col) in enumerate(self.clicks):
                ...
                cv2.circle(self.img, (lit_row, lit_col), 5, lit_color, 2)

                if shad_row is not None and shad_col is not None:
                    cv2.circle(self.img, (shad_row, shad_col), 5, shad_color, 2)
                    cv2.line(self.img, (lit_row, lit_col), (shad_row, shad_col), (255, 255, 255), 2)      
