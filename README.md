...
Rather, the program saves an ISD map for each processed image stored as a ```png```. The processed image (shawdow-free) can regenerated by processing images using the saved ISD map.

Prior to saving the ISD map, values are multiplied by 255.

To use the ISD maps downstream, map the ISD map to a float and divide by 255.
